...
class SelfOrderItem {
  ...
  factory SelfOrderItem.fromMap(Map<String, dynamic> map) {
    ...
    return SelfOrderItem(
      ...
      dineInQuantity:
          int.tryParse(map['dineInQuantity']?.toString() ?? '0') ?? 0,
      takeAwayQuantity:
          int.tryParse(map['takeAwayQuantity']?.toString() ?? '0') ?? 0,
  ...
  factory SelfOrder.fromFirestore(DocumentSnapshot doc) {
    try {
      ...
      return SelfOrder(
        ...
        isMember: false,
        memberId: '',
        namaCustomer: 'Gagal memuat pesanan',
        status: 'Gagal',
        orderItems: [],
        total: 0,
        directSubTotal: 0,
        directTakeAwayFee: 0,
        transactionMethod: 'Gagal',
        waktuPengambilan: '',
