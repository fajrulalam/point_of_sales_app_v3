...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance
        .addPostFrameCallback((_) => _measureDropdownOffset());
  }

  void _measureDropdownOffset() {
    final cardBox = _cardKey.currentContext?.findRenderObject() as RenderBox?;
    final rowBox =
        _employeeRowKey.currentContext?.findRenderObject() as RenderBox?;
    ...
    if (mounted) {
      setState(() {
        _dropdownTopOffset =
            (rowGlobal.dy - cardGlobal.dy) + rowBox.size.height;
      });
    }
  }

  void _onDigitTap(String digit) {
    if (_selectedEmployee == null || _enteredPin.length >= 6 || _isLoading)
      return;
    setState(() {
      _enteredPin += digit;
      _errorMessage = null;
  ...
  Future<void> _validateAndLogin() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _errorMessage = UserMessageService.fromError(
            e,
            fallback: 'Gagal masuk. Silakan coba lagi.',
          );
          _enteredPin = '';
          _isLoading = false;
        });
  ...
  Widget build(BuildContext context) {
    final otherEmployees =
        _employees.where((e) => e != _selectedEmployee).toList();
                          ...
                          Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                GestureDetector(
                                  ...
                                  child: Container(
                                    ...
                                    child: Row(
                                      children: [
                                        if (_selectedEmployee != null)
                                          _buildAvatar(_selectedEmployee!,
                                              isSelected: true)
                                        else
                                          Container(
                                            width: 36,
                                ...
                                Opacity(
                                  opacity:
                                      _selectedEmployee != null ? 1.0 : 0.45,
                                  child: IgnorePointer(
  ...
  Widget _buildDeleteKey() {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: const Center(
          child:
              Icon(Icons.backspace_outlined, size: 24, color: Colors.black54),
        ),
      ),
    );
  }
